...
// ---- public API ------------------------------------------------------------
/**
 * Collect newest items across all RSS/WP outlets **and** the X social tier,
 * de-duplicated (exact link/id + title fingerprint) and newest-first.
 * Returns { sources, articles, social }.
 */
export async function collectArticles() {
  const [feedResults, xResult] = await Promise.all([
    Promise.allSettled(SOURCES.map((s) => (s.type === "wp" ? collectWp(s) : collectFeed(s)))),
    // X is a separate, isolated module; never let a social failure sink the feed.
  ...
  const add = (a) => {
    if (!a.is_social && isJunk(a)) return false; // streamlined to tech: drop commerce/coupon noise
  ...
  // Articles first (publisher stories), then social — so an outlet's own write-up
  // of an announcement wins the fingerprint over the raw tweet.
