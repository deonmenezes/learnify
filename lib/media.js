...
const MODE = (process.env.TECHSCROLL_IMAGE_MODE || "gen").toLowerCase();
...
function genUrl(a) {
  ...
  const q = new URLSearchParams({
    width: "640", height: "400", nologo: "true", model: "flux",
...
function genMedia(a) {
  return { media_url: genUrl(a), media_kind: "ai", media_credit: "AI-generated · TechScroll" };
}
...
export async function attachMedia(list, base = "") {
  ...
  if (!usesExternal) return list.map((a) => ({ ...a, ...defaultMedia(a, base) }));
  const out = [];
