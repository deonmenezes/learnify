...
async function queryOpenAlexTopic(term, tag) {
  // ~7-month window so force-tagged topical papers stay RECENT (the old 900-day
  // window + cited-desc surfaced ancient, mega-cited off-topic papers that got
  // mis-binned). Relevance/categorization in api/research.js is the final guard.
  const filter = ["type:article", `from_publication_date:${daysAgoISO(210)}`].join(",");
  const url = `${OPENALEX}?search=${encodeURIComponent(term)}&filter=${encodeURIComponent(filter)}&sort=cited_by_count:desc&per_page=6&mailto=support@techscroll.app`;
  try {
    const j = JSON.parse(await getText(url, 10000));
    return (j.results || []).map((w) => {
      const m = mapOAWork(w, { trending: true });
      if (m) { m.categories = [tag, ...(m.categories || [])]; m.section = tag; }
      return m;
    }).filter(Boolean);
  } catch { return []; }
}
async function collectOpenAlexTopics() {
  const groups = await Promise.all(OA_TOPICS.map((t) => queryOpenAlexTopic(t.term, t.tag)));
  return groups.flat();
}
