// lib/papers.js — keyless research-paper collector for Learnify.
//
...
const CATS = [
  ...
  // Genre breadth (Learnify is a general learning app, not tech-only)
...
// Strict newest-first ordering shared by collector limits and the API. Missing
// or malformed dates sort after every valid date, while equal-date/undated rows
// retain their original provider order through the explicit index tie-break.
export function sortPapersNewestFirst(papers) {
  return papers.map((paper, index) => ({
    paper,
    index,
    publishedAt: parsePublicationDate(paper?.published)?.getTime() ?? null,
  })).sort((a, b) => {
...
// A publisher-hosted, freely readable copy of the work, if OpenAlex knows of
// one. Only absolute http(s) URLs are accepted; anything else is dropped rather
// than rendered as a broken or unsafe link.
function openAccessPdfUrl(work) {
  const candidates = [
...
export function mapOAWork(w, { trending = false, topic = null, now = new Date() } = {}) {
  ...
  if (Number.isNaN(anchor.getTime())) throw new TypeError("Invalid OpenAlex freshness anchor");
  const d = parsePublicationDate(w.publication_date);
  // OpenAlex occasionally indexes placeholder publication dates years ahead.
  // Reject them at the broad normalization boundary so they cannot outrank real
  // recent work in either the research endpoint or the mixed article feed.
  if (d && d.getTime() > anchor.getTime()) return null;
  const authors = (w.authorships || []).map((a) => a.author?.display_name).filter(Boolean);
  const author = authors.length > 2 ? `${authors[0]} +${authors.length - 1}` : authors.join(", ");
  const venue = w.primary_location?.source?.display_name || "";
  // OpenAlex already knows where a legally free copy lives. Surfacing it costs
  // nothing extra and is the difference between a reader opening the paper and
  // hitting a paywall. Link-out only: this is never fetched or rehosted.
  const oaPdf = openAccessPdfUrl(w);
...
const OA_TOPICS = [
  ...
  // Genre breadth — Learnify is a general learning app; research spans
  ...
  // Credible, peer-reviewed SCIENCE tier — the Science niche otherwise rides only
  ...
  // "science discovery" tag is whitelisted in research-shared.js and binned to the
  // app's "Science" category + genre by api/research.js.
...
export function mapOpenAlexTopicWork(work, { topicName, now = new Date() } = {}) {
  ...
  const paper = mapOAWork(work, { topic: topic.name, now });
...
export function buildOpenAlexTopicUrls(topicName, { now = new Date(), pageSize = 50 } = {}) {
  ...
  // Relevance-ranked search alone is not a newest-paper feed: an older, highly
  // relevant work can consume the whole candidate page before recent papers are
  // seen. Fetch a newest-ranked pool for every mapped query as well, then merge,
  // validate and deduplicate both pools in collectTopicPapers.
  return topic.queries.flatMap((query) =>
    ["publication_date:desc", "relevance_score:desc"].map((sort) => {
...
export async function collectTopicPapers(topicName, { now = new Date(), limit = 24 } = {}) {
  ...
  const papers = sortPapersNewestFirst([...byLink.values()]).slice(0, maxItems);
...
const ARXIV_GROUPS = [
  ...
  // Physical & life sciences, mind, money, space → Science / Finance genres
...
export function arxivGroupURL(cats, n) {
  const q = cats.map((c) => `cat:${c}`).join("+OR+");
  return `${ARXIV}?search_query=${q}&max_results=${n}&sortBy=submittedDate&sortOrder=descending`;
}
