// lib/x.js — keyless "X / social" collector for TechScroll.
//
...
// Curated voices. `weight` nudges the newsworthiness baseline: org accounts
// post real news more reliably than a personal account's stray musings.
export const X_ACCOUNTS = [
  { handle: "sama",         name: "Sam Altman",      org: "OpenAI",    weight: 4 },
  { handle: "AnthropicAI",  name: "Anthropic",       org: "Anthropic", weight: 10 },
  { handle: "claudeai",     name: "Claude",          org: "Anthropic", weight: 10 },
  { handle: "alexalbert__", name: "Alex Albert",     org: "Anthropic", weight: 5 }, // Claude dev relations
  { handle: "nvidia",       name: "NVIDIA",          org: "NVIDIA",    weight: 9 },
...
const MERGED_MAX = 24; // cap the social tier so it stays a tight "latest" set
...
export async function collectX(nowMs = (NOW_MS_DEFAULT ?? Date.now()), opts = {}) {
  ...
  if (opts.sequential) {
    ...
    const results = await Promise.allSettled(X_ACCOUNTS.map((a) => collectAccount(a, nowMs)));
    results.forEach((r, i) => {
      if (r.status !== "fulfilled" || !r.value.length) return;
      liveHandles.push(X_ACCOUNTS[i].handle);
      live.push(...r.value);
    });
  }
